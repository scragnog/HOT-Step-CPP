...
export const RefinePanel: React.FC = () => {
  ...
  const [narLrScale, setNarLrScale] = useState(1);
        ...
        <div className="mt-4 flex flex-col gap-4">
          ...
          <div className="rounded-lg border border-zinc-300/60 dark:border-white/5 p-3">
            ...
            <div className="mt-2 flex flex-wrap items-end gap-x-6 gap-y-3">
              ...
              <label className="flex flex-col gap-1 w-32">
                <ParamLabel label={t('trainingStudio.refine.narLrScale', 'Decoder rate (× run)')} meta={t('trainingStudio.refine.narLrMeta', '0.05–1 · default 1')} info={t('trainingStudio.refine.narLrInfo', 'Ceiling for the decoder pass, as a fraction of the source run\'s rate. The planner is frozen here, so the refinement\'s slow pacing (its rate box above, plus the halving on skipped rungs) does not apply. The pass ramps up to this over its first 30 steps and halves its rate on every gradient spike, so it settles just under what the decoder holds; at 1 a converged decoder typically spikes a few times early on. Lower it if the reconstruction meter climbs through the first checkpoints.')} />
                <input className={input} type="number" step="0.05" min={0.05} max={1} value={narLrScale} disabled={active || busy || !narFurther} onChange={e => setNarLrScale(Math.max(0.05, Math.min(1, Number(e.target.value) || 1)))} />
              </label>
