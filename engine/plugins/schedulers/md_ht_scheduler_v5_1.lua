...
-- MD HT Scheduler v5.1 -- HAP + TPT Thermodynamic Timestep Schedule
-- MDMAchine | A&E Concepts (c) 2026
--
-- Version: v5.1 (plugin and internal now aligned -- single version number)
--
...
-- verbose step-size diagnostics, auto-shift low-step compensation.
--
-- WHAT THIS DOES:
...
--        - Manual value from the Shift Warp slider, OR
--        - Auto-computed via Auto-Shift Ref (v5.1 addition, see below)
...
--
-- AUTO-SHIFT LOW-STEP COMPENSATION (v5.1):
--   Motivation: models tuned for a fixed reference step count (MM3 = 30)
--   degrade at low step counts in a specific way. With a uniform-ish schedule
--   and too few steps, the high-sigma region gets skimmed. On models that
--   denoise L/R channel halves independently (MM3) that under-resolves
--   low-frequency structure and lets the two channels do separate random
--   walks -- audible as thin, phasey, decorrelated stereo with weak bass
--   (measured: -8 dB at 60 Hz mid, L/R corr -0.07 vs +0.77 at 30 steps).
--   Fix: shift = (ref - 1) / (steps - 1). At steps==ref this is 1.0 (no-op).
--   At lower counts it warps spacing so short runs preserve the reference
--   trajectory's high-sigma dwell time -- bass and stereo correlation come
--   back. Ear-validated 8 to 30 steps; below 8 is extrapolation.
--   Tuning signature: muddy/smeared = shift too high, thin/wide = too low.
--
--   Auto-Shift Ref = 0 : off. Manual Shift Warp slider is used (v5.0 behavior).
--   Auto-Shift Ref > 0 : auto mode. Manual Shift is ignored. Set to the step
--                        count the target model was tuned at (MM3 = 30).
--   Auto value is clamped to [0.5, 8.0] to stay in the shift warp's safe range.
--
-- VERSIONING NOTE (v5.1):
--   Starting with this release, plugin version and internal version are
--   aligned as a single number. Previous scheme used a separate plugin
--   version (V1, V2, V3, V4) alongside an internal number (v4.x, v5.x).
--   From v5.1 onward: one version, one filename suffix, one changelog line.
--
-- CHANGELOG:
--   v5.1: Aligned plugin and internal versioning to a single number.
--         Added auto_shift_ref for automatic low-step compensation of
--         reference-tuned models (MM3 = 30). Overrides manual Shift when > 0.
--         Hardened shift fallback chain (params.shift or arg or 1.0) so no
--         branch can produce nil arithmetic. Extended verbose print with
--         active shift value and auto-shift status. No behavior change when
--         auto_shift_ref = 0. Drop-in safe on top of v5.0.
...
scheduler = {
    name        = "md_ht_scheduler v5.1",
    display     = "MD HT Scheduler (HAP+TPT) v5.1",
    description = "Thermodynamic timestep schedule: HAP + TPT additive density, density floor, LINA warp, SNR-space, poly slope, uniformity blend, smoothing, auto-shift compensation for low step counts. 12 to 150+ steps.",
    params      = {
        -- HAP ----------------------------------------------------------------
        ...
        { key = "shift", type = "slider", label = "Shift Warp",
          ...
          hint = "Native HOT-Step sigma warp. Applied first in the post-processing chain. Ignored when Auto-Shift Ref > 0." },
        { key = "auto_shift_ref", type = "slider", label = "Auto-Shift Ref",
          default = 0, min = 0, max = 60, step = 1,
          hint = "0=off (use manual Shift). Set to reference step count the model was tuned for (MM3=30). Auto-computes shift=(ref-1)/(steps-1) to restore bass and L/R correlation at low step counts. Overrides manual Shift when > 0. Clamped to [0.5, 8.0]." },
...
function schedule(output, num_steps, shift_val)
    ...
    local manual_sh = (params and params.shift)            or shift_val or 1.0
    local ref       = math.floor((params and params.auto_shift_ref) or 0)
    local verbose   = (params and params.verbose)          or false

    -- Auto-shift compensation: overrides manual shift when ref > 0.
    -- Formula: shift = (ref - 1) / (steps - 1). At steps==ref this is 1.0
    -- (no-op). At lower step counts it redistributes timestep spacing to
    -- preserve the reference trajectory's high-sigma dwell time, restoring
    -- low-frequency structure and stereo correlation on models that denoise
    -- L/R halves independently (MM3).
    local sh = manual_sh
    local auto_active = false
    if ref > 0 and num_steps > 1 then
        sh = (ref - 1) / (num_steps - 1)
        if sh < 0.5 then sh = 0.5 end   -- clamp to slider range for safety
        if sh > 8.0 then sh = 8.0 end
        auto_active = true
    end
    ...
    if verbose then
        print(string.format(
            "[HT v5.1] %d steps | ke=%.2f df=%.1f tc=%.2f pi=%.1f ww=%.2f fl=%.2f | snr=%s shift=%.3f%s lina=%.2f poly=%.2f ub=%.2f sm=%d",
            num_steps, ke, df, tc_frac, pi_, ww, fl,
            snr_mode and "on" or "off",
            sh, auto_active and string.format(" (auto ref=%d)", ref) or " (manual)",
            lina, poly, u_blend, sm_win))
        ...
        print(string.format("[HT v5.1] Gap range: min=%.5f max=%.5f ratio=%.1f:1",
