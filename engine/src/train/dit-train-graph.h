...
// dit_flash_probe / dit_flash_last_prec / dit_flash_prec_label used to be
// defined below; the AS1.5 LM port needs the same two answers, so they were
// MOVED (verbatim, same names) into this header rather than copied. See
// train/flash-prec.h and docs/plans/2026-09-02-lm-flash-attn.md D5/D6.
#include "train/flash-prec.h"
...
// spec §9.8's `dit_flash_probe` and the resolved-precision readback
// (`dit_flash_last_prec` / `dit_flash_prec_label`) now live in
// train/flash-prec.h, included above. They were moved there unchanged when the
// AS1.5 LM trainer gained the same flag, so both trainers ask the backend the
// same question through the same code.
