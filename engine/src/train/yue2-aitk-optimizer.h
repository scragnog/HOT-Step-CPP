...
struct ParameterSpec {
    ...
    // Multiplies StepConfig::learning_rate for this parameter only (the
    // planner/decoder split). 1.0 = the shared rate.
    float lr_scale = 1.0f;
};
...
public:
    ...
    void step_once(const StepConfig & cfg) {
        ...
        for (size_t i = 0; i < states.size(); ++i) {
            configs.push_back(make_config(slots_[i].elements, cfg, step_ + 1, specs_[i].lr_scale));
...
private:
    ...
    static aitk_adamw8bit::AdamW8bitCudaConfig make_config(size_t n, const StepConfig & c, int step, float lr_scale = 1.0f) { aitk_adamw8bit::AdamW8bitCudaConfig x; x.elements=n; x.step=step; x.learning_rate=c.learning_rate*lr_scale;x.beta1=c.beta1;x.beta2=c.beta2;x.eps=c.eps;x.weight_decay=c.weight_decay; return x; }
