...
| **LM and DiT adapters** | LoRA and LoKr on both, applied at runtime with live per-group strength dials, or merged into the weights for zero per-step cost. The DiT trainer also offers DoRA, rsLoRA, LoRA+, HiRA and LoHa parameterizations; the LM trainer adds learned artist tokens and a trainable KV prefix, stored in the same adapter file as the LoRA and applied automatically at generation. |
...
| **Flash-attention LM training** | The same fused attention op, ported to the planner LM trainer, with the causal mask skipping roughly half its compute for free. Raises the longest song a 0.6B/1.7B adapter trains on without truncation, and is measurably faster than the shipped per-head-block path on the 4B. Off by default in the Training Studio checkbox pending an ear test; the exact graph is what every adapter has trained on so far. |
